...
def solve_optimal_focal_shift(uv: np.ndarray, xyz: np.ndarray):
    """
    Solve `min |focal * (xy + offset) / (z + shift) - uv|`
    with respect to shift, focal, and xy offset.
    Handles point maps where the 3D centroid has been shifted away from the optical axis.
    """
    ...
    # Closed-form init via linearized 4-param system:
    #   focal*(x+ox) - shift*u = u*z  →  focal*x + p - shift*u = u*z  (p = focal*ox)
    #   focal*(y+oy) - shift*v = v*z  →  focal*y + q - shift*v = v*z  (q = focal*oy)
    A_lin = np.zeros((2 * N, 4), dtype=xyz.dtype)
    A_lin[0::2, 0] = xy[:, 0]
    A_lin[1::2, 0] = xy[:, 1]
    A_lin[0::2, 1] = -uv[:, 0]
    A_lin[1::2, 1] = -uv[:, 1]
    A_lin[0::2, 2] = 1.0
    A_lin[1::2, 3] = 1.0
    b_lin = np.empty(2 * N, dtype=xyz.dtype)
    b_lin[0::2] = uv[:, 0] * z
    b_lin[1::2] = uv[:, 1] * z
    sol_lin, _, _, _ = np.linalg.lstsq(A_lin, b_lin, rcond=None)
    f_init = float(sol_lin[0])
    s_init = float(sol_lin[1])
    ox_init = float(sol_lin[2]) / (abs(f_init) + 1e-12)
    oy_init = float(sol_lin[3]) / (abs(f_init) + 1e-12)

    x0 = np.array([max(shift_lb + 0.01, s_init), ox_init, oy_init])

    def fn(uv, xy, z, params):
        shift, ox, oy = params[0], params[1], params[2]
        xy_off = xy + np.array([[ox, oy]])
        xy_proj = xy_off / (z + shift)[:, None]
        f = (xy_proj * uv).sum() / np.square(xy_proj).sum()
        return (f * xy_proj - uv).ravel()

    solution = least_squares(
        partial(fn, uv, xy, z), x0=x0, ftol=1e-3, method='trf',
        bounds=([shift_lb, -np.inf, -np.inf], [np.inf, np.inf, np.inf]),
        loss='soft_l1',
    )
    optim_shift = float(solution['x'][0])
    optim_offset = solution['x'][1:3].astype(np.float32)

    xy_off = xy + optim_offset
    xy_proj = xy_off / (z + optim_shift)[:, None]
    optim_focal = float((xy_proj * uv).sum() / np.square(xy_proj).sum())

    return np.float32(optim_shift), np.float32(optim_focal), optim_offset


def solve_optimal_shift(uv: np.ndarray, xyz: np.ndarray, focal: float):
    """
    Solve `min |focal * (xy + offset) / (z + shift) - uv|`
    with respect to shift and xy offset (focal is given).
    """
    from scipy.optimize import least_squares
    uv, xy, z = uv.reshape(-1, 2), xyz[..., :2].reshape(-1, 2), xyz[..., 2].reshape(-1)
    N = len(z)
    ...
    # Closed-form init: focal*(x+ox) - shift*u = u*z, focal*(y+oy) - shift*v = v*z
    # Unknowns: [shift, focal*ox, focal*oy]
    A_lin = np.zeros((2 * N, 3), dtype=xyz.dtype)
    A_lin[0::2, 0] = -uv[:, 0]
    A_lin[1::2, 0] = -uv[:, 1]
    A_lin[0::2, 1] = 1.0
    A_lin[1::2, 2] = 1.0
    b_lin = np.empty(2 * N, dtype=xyz.dtype)
    b_lin[0::2] = uv[:, 0] * z - focal * xy[:, 0]
    b_lin[1::2] = uv[:, 1] * z - focal * xy[:, 1]
    sol_lin, _, _, _ = np.linalg.lstsq(A_lin, b_lin, rcond=None)
    s_init = float(sol_lin[0])
    ox_init = float(sol_lin[1]) / (abs(focal) + 1e-12)
    oy_init = float(sol_lin[2]) / (abs(focal) + 1e-12)
    ...
    return np.float32(optim_shift), optim_offset
