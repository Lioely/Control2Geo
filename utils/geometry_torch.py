...
def recover_focal_shift(points: torch.Tensor, mask: torch.Tensor = None, focal: torch.Tensor = None, downsample_size: Tuple[int, int] = (64, 64)):
    """
    Recover the best-fit virtual camera (focal, z-shift, xy-offset) from a point map,
    fitting `uv = focal * (xy + offset) / (z + shift)`.

    Handles point maps whose 3D centroid has been shifted away from the optical axis
    (e.g. after affine-invariant normalization).
    ...
    - `shift`: torch.Tensor of shape (...) Z-axis shift
    - `offset`: torch.Tensor of shape (..., 2) XY offset to add to point XY before projection
    """
    shape = points.shape
    ...
    optim_shift, optim_focal, optim_offset = [], [], []
    for i in range(points.shape[0]):
        ...
        if uv_lr_i_np.shape[0] < 2:
            ...
            optim_offset.append([0.0, 0.0])
            continue
        if focal is None:
            shift_i, focal_i, offset_i = solve_optimal_focal_shift(uv_lr_i_np, points_lr_i_np)
            optim_focal.append(float(focal_i))
        else:
            shift_i, offset_i = solve_optimal_shift(uv_lr_i_np, points_lr_i_np, focal_np[i])
        optim_shift.append(float(shift_i))
        optim_offset.append([float(offset_i[0]), float(offset_i[1])])
    optim_shift = torch.tensor(optim_shift, device=points.device, dtype=points.dtype).reshape(shape[:-3])
    optim_offset = torch.tensor(optim_offset, device=points.device, dtype=points.dtype).reshape(*shape[:-3], 2)
    ...
    return optim_focal, optim_shift, optim_offset
